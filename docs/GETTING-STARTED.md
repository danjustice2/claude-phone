...
A "device" is a Claude personality that answers on a SIP extension.

```
? Device name (e.g., Morpheus): Morpheus
```
A friendly name for your AI assistant. Call it whatever you like.

```
? SIP extension number (e.g., 9000): 9000
```
The number you'll dial to reach this device. **9000** is the default.

```
? SIP auth ID: 9000
```
Used for SIP authentication. **Use the same value as the extension** (e.g., `9000`).

```
? SIP password: ****
```
A password for this device's SIP registration. Pick something (e.g., `claude9000`). This is internal — only used between Docker containers.
...
Now configure your softphone to register with Asterisk. Use these settings:
...
Extensions 1001 and 1002 are pre-configured user phone extensions. To change the password, set `USER_EXT_PASSWORD` in your `.env` file and restart.
